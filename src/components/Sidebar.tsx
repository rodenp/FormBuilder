...
const SidebarItem: React.FC<SidebarItemProps> = ({ type, label, icon, description }) => {
    ...
    return (
        <div
            ...
            {...attributes}
            className={`
        group relative flex flex-col items-center gap-2 p-3 bg-white border border-slate-200 rounded-xl cursor-move shadow-sm
        hover:border-brand-500 hover:shadow-md hover:shadow-brand-500/10 transition-all duration-200 ease-in-out
